...
import {
  Box,
  Typography,Button,Dialog,DialogTitle,DialogContent,DialogActions,TextField,FormControl,InputLabel,Select,MenuItem,IconButton,Chip,Alert,CircularProgress,InputAdornment,Tooltip,Grid,Card, CardMedia,CardContent,} from '@mui/material';
import {Add as AddIcon, Edit as EditIcon, Delete as DeleteIcon, Search as SearchIcon, LocalOffer as PriceIcon, Timer as TimerIcon, Category as CategoryIcon, } from '@mui/icons-material';
  ...
  return (
    <Box>
      ...
      <Box sx={{ mb: 4, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <Typography
          variant="h5"
          sx={{color: '#E65100', fontWeight: 700, letterSpacing: '0.5px', textTransform: 'uppercase', position: 'relative',
            '&::after': { content: '""', position: 'absolute',bottom: -8, left: 0,width: '40px', height: '3px', background: '#E65100', borderRadius: '2px',},
          }}
        >
          Menu Items
        </Typography>
        <Button
          ...
          sx={{backgroundColor: '#E65100', color: 'rgba(255, 255, 255, 0.95)', fontWeight: 600, letterSpacing: '0.5px', textTransform: 'none', padding: '8px 24px', borderRadius: 2, boxShadow: '0 4px 12px rgba(230, 81, 0, 0.3)',
      ...
      <TextField
        ...
        sx={{mb: 3,
          '& .MuiOutlinedInput-root': { color: 'rgba(255, 255, 255, 0.95)', background: 'rgba(255, 255, 255, 0.05)', borderRadius: 2,
            transition: 'all 0.3s ease',
            '& fieldset': { borderColor: 'rgba(255, 255, 255, 0.1)', },
            '&:hover fieldset': { borderColor: 'rgba(255, 255, 255, 0.2)',},
            '&.Mui-focused fieldset': {
              borderColor: '#E65100', boxShadow: '0 0 0 2px rgba(230, 81, 0, 0.2)',},
          },
          '& .MuiInputLabel-root': { color: 'rgba(255, 255, 255, 0.7)',
            '&.Mui-focused': {color: '#E65100',},
          },
        }}
        InputProps={{
          startAdornment: (
      ...
      <Grid container spacing={3}>
        {filteredItems.map((item) => (
          <Grid item xs={12} sm={6} md={4} key={item._id}>
            <Card
              sx={{height: '100%',display: 'flex',flexDirection: 'column',background: 'rgba(30, 30, 30, 0.95)',backdropFilter: 'blur(10px)',border: `1px solid rgba(255, 255, 255, 0.1)`, borderRadius: 2,transition: 'all 0.3s ease',
                '&:hover': {transform: 'translateY(-4px)',boxShadow: '0 8px 24px rgba(230, 81, 0, 0.2)',borderColor: 'rgba(230, 81, 0, 0.3)',
                },
              }}
            >
              <CardMedia
              ...
              <CardContent sx={{ flexGrow: 1 }}>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 2 }}>
                  <Typography
                    variant="h6"
                    sx={{color: 'rgba(255, 255, 255, 0.95)',fontWeight: 600,letterSpacing: '0.3px', }}
                  >
                    {item.name}
                  </Typography>
                  <Chip
                ...
                <Typography
                  variant="body2"
                  sx={{color: 'rgba(255, 255, 255, 0.7)',mb: 2,minHeight: '40px',
                  }}
                >
                  {item.description}
                </Typography>
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 2 }}>
                  <Chip
                    ...
                    sx={{background: 'rgba(230, 81, 0, 0.2)',color: '#E65100',fontWeight: 600,
                    }}
                  />
                ...
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mt: 'auto' }}>
                  <Typography
                    sx={{color: '#E65100',fontWeight: 700,fontSize: '1.2rem',display: 'flex',alignItems: 'center',gap: 1,}}
                  >
                    ...
                    <Tooltip title="Edit Item">
                      <IconButton
                        ...
                        sx={{color: 'rgba(255, 255, 255, 0.7)',
                          '&:hover': {color: '#E65100',background: 'rgba(230, 81, 0, 0.1)',},
                        }}
                      >
                        <EditIcon />
                      </IconButton>
      ...
      {filteredItems.length === 0 && (
        <Box
          sx={{textAlign: 'center',mt: 4,p: 4,background: 'rgba(30, 30, 30, 0.8)',borderRadius: 2,border: '1px solid rgba(255, 255, 255, 0.1)',}}
        >
          <Typography
            sx={{color: 'rgba(255, 255, 255, 0.7)',fontWeight: 500,letterSpacing: '0.5px',}}
          >
            No menu items found
          </Typography>
        </Box>
